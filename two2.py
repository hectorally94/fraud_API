...
def reportClasification(): 
    ...
    try:
        ...
        cursor.execute("SELECT m.id As declarationId,  m.taxdeclared, m.datepayed,m.enterpriseid, c.id As chiffreAfaireId, c.enterprisename, c.chifreaffaire, c.datatopay FROM declaration m INNER JOIN chiffreaffaire c ON c.id = m.id")
        declarations = cursor.fetchall()
        global dataD
        dataD = pd.DataFrame(declarations)
        dataD['taxdeclared'] = dataD['taxdeclared'].astype(float)
        #dataD['datepayed'] = dataD['datepayed'].astype('datetime64[ns]')
        print (dataD)
        print (dataD.dtypes)
        #print(dataD)
        global combination
